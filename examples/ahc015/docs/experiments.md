...
## 2026-09-07 未来列なし128 channel・蒸留なしPPO 10時間（結果）

- W&B: `large-20260906-043823`（run ID `oiieuj16`）、正常終了
- 実行時間: 10.097時間、epoch 320から402、追加33,251,328 environment transitions、32,472 update
- 固定2,048ケース評価: 開始checkpointの791,250.972に対し、epoch 371（6.28時間）でrun内bestの
  796,200.133 ±1,733.542、最終epoch 402で791,590.136 ±1,707.041だった。開始点に対してbestは
  +4,949.161、lastは+339.164。前runのbest 792,080.117も+4,120.016更新した
- PPO状態: 最終approximate KL 0.00558、clip fraction 0.0477、explained variance 0.9842、
  rollout entropy 0.1627。数値的な発散はないが、best到達後の約3.8時間は横ばいで、lastはbestより
  4,609.998低かった
- checkpoint: W&Bのbest training checkpointはepoch 371。ユーザー指定に従い継続に用いる`last.pt`は
  epoch 402、optimizer state 88 parameter分を保持し、SHA-256は
  `2ff4648c0e014def55ee73f04f953a9915ded639b28eecc2b72fc7964557d76d`。artifactは
  `large-20260906-043823-last-training-checkpoint:v0`
- 次段階: 上記lastから9時間30分継続する。飽和に近づいた一方、KLは過大ではないため、大幅な減衰は
  避けて学習率だけを`3e-4`から`2.5e-4`へ下げる。entropy係数`0.01`など他条件は維持し、rollout
  seedを`15047`へ変更する。Notebookは`ahc015-128-ppo.ipynb`

## 2026-09-07 提出用終盤expectimax

- モデル: 完了済み128 channel runの固定評価best、W&B
  `large-20260906-043823-training-checkpoint:v0`のepoch 371（796,200.133 ±1,733.542）
- 実装: 4特徴面・128 channel・10 block afterstate actorをRustへ移植してq8 export。
  終盤は将来の一様ランダム配置を全列挙し、配置後の行動を最大化する。確率分母が候補間で共通のため、
  終端連結度分子の整数和で厳密比較する。盤面と手数をkeyにtransposition tableを使う
- 1,000ケース（seed `20260907`、10並列）: 探索なし798,044.324 ±2,437.522、4ターン
  813,850.597 ±2,157.378（+15,806.273 ±949.969）、5ターン817,195.383 ±2,118.570、
  6ターン820,155.768 ±2,069.543
- paired 300ケース（seed `20260909`）: 6ターンは探索なし比+20,670.320 ±1,985.032、
  改善56.3%、悪化11.7%、同点32.0%
- 10並列300ケースの個別latency: 6ターンは平均0.792秒、p95 0.851秒、p99 0.878秒、最大0.907秒。
  7ターンは平均1.335秒、p95 1.758秒、p99 1.997秒、最大2.130秒で制限超過例があった
- 判断: 既定値は余裕のある6ターン。7ターンは平均では2秒以内だがtail latencyが危険なため不採用

## 2026-09-07 ルール方策による終盤モンテカルロ探索

- playout方策: 抽象味0を下、1を左上、2を右上へ割り当てる。異なる味への遷移が1/2→0なら上、
  0→1/2なら下、1→2なら右、2→1なら左。味0の連続中は、次の異なる味が1なら右、2なら左へ
  傾け、実際の境界では通常の下方向を使う。
  味の役割6 permutationと方向4 rotationの24ルールを実装した
- common random numbers: sampleごとに未来の一様ランダム配置rank列を1本生成し、比較対象の初手方向と
  24ルールすべてへ同じ列を適用する
- 比較: 20/30ターンwindowは不安定または悪化し、12ターンwindowが良かった。初手はモデル上位2方向
  より4方向全てを試す方が改善した。successive halvingは均等samplingとほぼ同等だったが、時間に
  余裕があるため平均がわずかに高い均等samplingを採用した
- 調整300ケース（seed `20260919`）: 12ターンwindow・4方向・覆し閾値20で、64/128/256/512
  sampleのpaired差は+3,257.687/+4,776.400/+3,561.220/+4,890.753。128 sampleは512と同等で高速
- 未使用1,000ケース（seed `20260920`）: 6ターン厳密探索のみ818,073.390 ±2,072.951、採用MCは
  823,392.115 ±1,867.576、paired差+5,318.725 ±1,104.152。改善35.8%、悪化29.9%、同点34.3%
- 10並列1,000ケース: 採用MCのジャッジ相当平均0.928秒。個別latencyは平均0.925秒、p95 0.990秒、
  p99 1.022秒、最大1.100秒（厳密探索のみは平均0.792秒、最大0.987秒）
- 採用: 終盤12ターンwindowのうち88〜93手目にMC、94手目以降は6ターン厳密探索。初手4方向、
  24ルール均等sampling、各128 sample、覆し閾値20。固定sample上限に加え、残時間を残りMC手数で割る
  deadlineを設ける

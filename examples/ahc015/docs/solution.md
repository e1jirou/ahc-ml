...
- 86〜92手目は、確率的な配置を含むMCTSで初手4方向を比較する。配置後盤面をhash keyにして、異なる経路から
  ...
  要求する。根の各候補は、次の全配置rankに対応する決定ノードごとに観測済み行動の最大平均価値を取り、その
  一様平均で比較する。未観測の配置rankだけは従来の根playout平均へfallbackする。この1段backupにより、PUCTの
  探索行動を含む単純平均ではなく、次の配置を見た後に最善行動を選ぶ実際のゲーム進行を反映する。
  直後2配置のrank組は一巡まで重複しないよう層化する。playout完了後は、そのsimulationで判明した

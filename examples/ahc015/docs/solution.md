...
1. **近未来の集約特徴を追加する。** 今後5、10、20手に出る各味の個数と次回出現までの距離を明示し、
   現在のfuture sequence用FCだけに集約を任せる場合と比較する。
2. **モデル変更は同一条件の対照runで比較する。** spatial poolingの再検証を行う場合は、同一checkpoint・
   seed・学習時間のglobal average版も実行し、単なる継続学習の揺らぎと構造変更の効果を分離する。
3. **複数seed評価をbest選択に使う。** 固定512ケース1組だけでなく、複数seedを交互に評価し、単一集合への
   過適合とcheckpoint偶然差を抑える。これは学習能力を直接改善しないため、80万点を狙う段階では
   モデル・特徴量の実験より優先しない。
